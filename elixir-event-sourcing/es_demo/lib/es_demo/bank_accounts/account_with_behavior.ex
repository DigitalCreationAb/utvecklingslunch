defmodule EsDemo.BankAccounts.AccountWithBehavior do
  ...
  defmodule NotOpened do
    ...
    def execute_call(_command, _from, _state) do
  ...
  defmodule Closed do
    def execute_call({:deposit, %{}}, _from, _context) do
      {:error, "Can't deposit money to a closed account"}
    end

    def execute_call({:withdraw, %{}}, _from, _context) do
      {:error, "Can't withdraw money from a closed account"}
    end

    def execute_call(:close, _from, _context) do
      {:error, "This account is already closed"}
    end

    def execute_call(:suspend, _from, %{}) do
      {:error, "Can't suspend a closed account"}
    end

    def execute_call(:get_balance, _from, %{id: id}) do
      %{id: id, balance: 0}
    end
  end

  defmodule Suspended do
    def execute_call(:restore, _from, %{id: id}) do
      {[{:account_restored, %{id: id, time_stamp: DateTime.utc_now()}}], %{id: id}}
    end

    def execute_call({:deposit, %{}}, _from, _state) do
      {:error, "Can't deposit money into a suspended account"}
    end

    def execute_call({:withdraw, %{}}, _from, _state) do
      {:error, "Can't withdraw money from a suspended account"}
    end

    def execute_call(:close, _from, _state) do
      {:error, "Can't close a suspended account"}
    end

    def execute_call(:suspend, _from, _state) do
